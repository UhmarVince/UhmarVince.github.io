...
function showFixedBeamDiag() {
    ...
    // Maximum moment and shear for fixed beam with UDL
    let maxMoment = (varLoadValue * Math.pow(varLengthValue, 2)) / 12; // At the fixed ends
    let maxShear = (varLoadValue * varLengthValue) / 2; // At the fixed ends

    // Update shear diagram
    let distances = [
        0,
        0.125 * varLengthValue,
        0.25 * varLengthValue,
        0.375 * varLengthValue,
        0.5 * varLengthValue,
        0.625 * varLengthValue,
        0.75 * varLengthValue,
        0.875 * varLengthValue,
        varLengthValue
    ];

    let shears = distances.map((x) => {
        return maxShear - varLoadValue * x;
    });
    ...
    let moments = distances.map((x) => {
        return (
            (maxMoment - ((varLoadValue * x) * (varLengthValue - x)) / 2) // Moment equation for a fixed-end beam
        );
    })

    return Moment Chart if value dist return ==suport layout testing tool map constant workable SNR N        
...
/*
    ...
    options: {
        ...
        function changedSupport(){
        const changedCanvas = document.getElementById("shearDiagram"); 
    
        // Clear the Chart.js instance if it exists
        if (chartChange) {
            chartChange.destroy(); // Destroy the existing chart
            chartChange = null;    // Reset the chart instance
        }
    
        // Hide the canvas
        if (changedCanvas) {
            changedCanvas.style.display = "none"; // Hides the canvas
        }
    
        }
    
        function changedSupport1(){
            const changedCanvas1 = document.getElementById("momentDiagram"); 
        
            // Clear the Chart.js instance if it exists
            if (chartChange1) {
                chartChange1.destroy(); // Destroy the existing chart
                chartChange1 = null;    // Reset the chart instance
            }
        
            // Hide the canvas
            if (changedCanvas1) {
                changedCanvas1.style.display = "none"; // Hides the canvas
            }
        ...
        supportive.addEventListener('change', changedSupport);
        supportive1.addEventListener('change', changedSupport); 
        supportive.addEventListener('change', changedSupport1);
        supportive1.addEventListener('change', changedSupport1);
